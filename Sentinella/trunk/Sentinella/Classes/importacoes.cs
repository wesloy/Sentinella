...
namespace Sentinella {
    ...
    class importacoes {
        ...
        public importacoes(string _Bin, string _Cpf, DateTime dth_registro, int _Fila_id, DateTime dth_abertura, int idHistorico = 0, int idInfo = 0) {
            ...
            Id_abertura = Constantes.idlogadoFerramenta;
            ...
            try {
                ...
                foreach (DataRow row in dt.Rows) {
                    ...
                    while (dr.Read()) {
                        ...
                        DateTime dateValue;
                        if (DateTime.TryParse(dr["Generated"].ToString().Replace("'", ""), out dateValue)) {
                            data = dateValue;
                        } else {
                            volTotal += 1;
                            frm.atualizarBarra(volTotal);
                        ...
                        if (DateTime.TryParse(dr["Received"].ToString().Replace("'", ""), out dateValue)) {
                            data = dateValue;
                        } else {
                            volTotal += 1;
                            frm.atualizarBarra(volTotal);
        ...
        private bool _cadastroGeralConexao(string diretorio, ref long volAtualizado) {
            ...
            try {
                ...
                while (dr.Read()) {
                    ...
                    //    sql += "idAtualizacao = " + objCon.valorSql(Constantes.idlogadoFerramenta) + " ";
