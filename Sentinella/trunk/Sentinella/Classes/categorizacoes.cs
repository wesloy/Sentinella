...
namespace Sentinella {
    class categorizacoes {
        ...
        public string IdCat {
            get {
                return objCon.retornaVazioParaValorNulo(_idCat).ToString().Replace("  ", " ").Trim();
            }
        ...
        public categorizacoes(int _id,  int _finalizacao_id = 0, int _subFinalizacao_id = 0, double _valorEnvolvido = 0, string _observacao = "") {
            ...
            Id = _id;
            Status_id = 3;
            IdCat = Constantes.idlogadoFerramenta;
            Fila_id = cat.Fila_id;
            ...
            try {
                ...
                if (dt.Rows.Count > 0) {
                    foreach (DataRow ln in dt.Rows) {
                        ...
                        registro.IdCat = ln["IdCat"].ToString();
        ...
        private bool _finalizarBaseMassa(categorizacoes obj, ref long volFinalizado) {
            ...
            try {
                ...
                sql = "Update w_base set ";
                sql += "status_id = 3, ";
                sql += "idCat = " + objCon.valorSql(Constantes.idlogadoFerramenta) + ", ";
                sql += "finalizacao_id = " + objCon.valorSql(obj.Finalizacao_id) + ", ";
                sql += "subFinalizacao_id = " + objCon.valorSql(obj.SubFinalizacao_id) + ", ";
                ...
                if (fups) {
                    ...
                    sql += "and idCat = " + objCon.valorSql(Constantes.idlogadoFerramenta) + " ";
        ...
        private bool _validacaoRegistroLocado(ref categorizacoes registro) {
            try {
                ...
                sql = "Select top 1 id from w_base where idCat = " + objCon.valorSql(Constantes.idlogadoFerramenta) + " ";
                sql += "and status_id <> 3 ";
                sql += "order by id";
                dt = objCon.retornaDataTable(sql);
                if (dt.Rows.Count > 0) {
            ...
            try {
                ...
                if (!validacao) {
                    MessageBox.Show("Não foi possível selecionar nenhum registro para trabalho, possível motivo: " + Environment.NewLine +
                                        " - Não há mais registros disponíveis para a fila selecionada!", Constantes.Titulo_MSG, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                }
                return validacao;
            } catch (Exception ex) {
