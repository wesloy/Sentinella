        ...
        Try
            With exec_DTO
                ...
                Else
                    ...
                    'Manter um histórico mínimo de 12 meses
                    sql = "select cpf, cartao,  max(dataCorte) as dtCorte, max(dataAtualizacao) as dtAtualizacao from w_faturas "
                ...
                If Not TelaCNPBCOT1 Is Nothing Then
                    ...
                    Do While rsCards.EOF() = False
                        rsDados.Filter = ""
                        rsDados.Filter = "cpf = '" & rsCards.Fields("CPF").Value & "' and cartao = '" & rsCards.Fields("cartao").Value & "'"
                        Dim dataUltAtualizacao As Date = "01/01/1900"
                        If rsDados.RecordCount = 0 Then
                            'Quando não tem dados de manutenções capturados
                            'Selecionar o período máximo
                            'Usado 13 meses para evitar problemas na captura do 12º mês devido ao dia do corte
                            dataLimite = Date.Now.AddMonths(-13).Date
                            dataUltAtualizacao = "01/01/1900"
                        Else
                            dataLimite = rsDados.Fields("dtCorte").Value
                            dataUltAtualizacao = hlp.FormataDataAbreviada(rsDados.Fields("dtAtualizacao").Value)
                            dataUltAtualizacao.AddDays(-5) 'Como o processo é moroso e a tela preta cai, considerar 3 dias para atualização total do bd
                        End If

                        'Em situações em que o robo cai e é reiniciado, evitando captura de cartoes já trabalhados
                        If dataUltAtualizacao < Today Then
